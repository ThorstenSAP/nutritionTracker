 ...
 (function($){
		...
		return this.each(function(){
			...
			if ($('#'+id).length == 0) {
				...
				if (options.onKeypadOpen){
					nmpd.on('numpad.open', function(Event){
						$(document).blur()
					});
				}
	...
	*/
	...
	$.fn.numpad.defaults = {
		target: false,
		openOnEvent: 'click',
		backgroundTpl: '<div class="modal-backdrop in nmpd-overlay"></div>',
		gridTpl: '<table class="table modal-content nmpd-grid"></table>',
		displayTpl: '<input type="text" class="form-control  input-lg nmpd-display" />',
